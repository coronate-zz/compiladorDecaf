...
void EmptyExpr::impresionAux(int indentLevel) {
...
void IntConstant::impresionAux(int indentLevel) {
    printf("%d", value);
    if (expr_type) std::cout << " <" << expr_type << ">";
    if (emit_loc) emit_loc->Print();
}
...
void DoubleConstant::impresionAux(int indentLevel) {
    printf("%g", value);
    if (expr_type) std::cout << " <" << expr_type << ">";
    if (emit_loc) emit_loc->Print();
}
...
void BoolConstant::impresionAux(int indentLevel) {
...
void StringConstant::impresionAux(int indentLevel) {
    printf("%s",value);
    if (expr_type) std::cout << " <" << expr_type << ">";
    if (emit_loc) emit_loc->Print();
}
...
void NullConstant::impresionAux(int indentLevel) {
    if (expr_type) std::cout << " <" << expr_type << ">";
    if (emit_loc) emit_loc->Print();
}
...
void Operator::impresionAux(int indentLevel) {
...
void CompoundExpr::impresionAux(int indentLevel) {
    if (expr_type) std::cout << " <" << expr_type << ">";
    if (emit_loc) emit_loc->Print();
...
void This::impresionAux(int indentLevel) {
    if (expr_type) std::cout << " <" << expr_type << ">";
}

void This::CheckType() {
    Decl *d = symtab->recorrerHashThis();
    if (!d || !d->IsClassDecl()) {
...
void ArrayAccess::impresionAux(int indentLevel) {
    if (expr_type) std::cout << " <" << expr_type << ">";
    if (emit_loc) emit_loc->Print();
...
void FieldAccess::impresionAux(int indentLevel) {
    if (expr_type) std::cout << " <" << expr_type << ">";
    if (emit_loc) emit_loc->Print();
...
void FieldAccess::CheckDecl() {
    if (!base) {
        Decl *d = symtab->recorrerHash(field);
        if (d == NULL) {
...
void FieldAccess::CheckType() {
    ...
    if (base_t != NULL) {
        ...
        Decl *d = symtab->recorrerHashField(
        ...
        if (d == NULL || !d->IsVarDecl()) {
            ...
            Decl *cur_class = symtab->recorrerHashThis();
...
void Call::impresionAux(int indentLevel) {
    if (expr_type) std::cout << " <" << expr_type << ">";
    if (emit_loc) emit_loc->Print();
    if (base) base->Print(indentLevel+1);
    field->Print(indentLevel+1);
...
void NewExpr::impresionAux(int indentLevel) {
    if (expr_type) std::cout << " <" << expr_type << ">";
    if (emit_loc) emit_loc->Print();
...
void NewArrayExpr::impresionAux(int indentLevel) {
    if (expr_type) std::cout << " <" << expr_type << ">";
    if (emit_loc) emit_loc->Print();
...
void PostfixExpr::impresionAux(int indentLevel) {
    if (expr_type) std::cout << " <" << expr_type << ">";
    if (emit_loc) emit_loc->Print();
