...
%type <exprList_union>                  Actuals_EpsilonComa
...
%type <interfaceList_union>             Interface_ComaIdentifier
...
InterfaceList   : T_Identifier Interface_ComaIdentifier         { ($$ = $2)->InsertAt(new NamedType(new Identifier(@1, $1)), 0); }
                ;

Interface_ComaIdentifier  : ',' T_Identifier Interface_ComaIdentifier     { ($$ = $3)->InsertAt(new NamedType(new Identifier(@2, $2)), 0); }
            |                                                   { $$ = new List< NamedType* >(); }
            ;
...
PrintList  : Expr Actuals_EpsilonComa                        { ($$ = $2)->InsertAt($1, 0); }
>>>>>>> 2aaed1dfd717e8fb500d18ee0b5b355dcd64c25b
          ;
...
Call  : T_Identifier '(' Actuals ')'                          {
      ...
      | Expr '.' T_Identifier '(' Actuals ')'                 { // Cuando usamos atributos de objetos
...
Actuals : Expr Actuals_EpsilonComa                            { ($$ = $2)->InsertAt($1, 0); }
        |                                                     { $$ = new List< Expr* >(); }
        ;

Actuals_EpsilonComa   : ',' Expr Actuals_EpsilonComa          { ($$ = $3)->InsertAt($2, 0); }
                     |                                        {  $$ = new List< Expr* >(); }
                     ;
